...
app.get("/random", (req, res) => {
  // Generate a random index within the jokes array length
  const randomIndex = Math.floor(Math.random() * jokes.length);
  // Return the joke at the random index as JSON
...
app.get("/jokes/:id", (req, res) => {
  ...
  // Search for a joke with matching id
  const findjoke = jokes.find(joke => joke.id === jokeId);
  // Check if joke was found
  if (findjoke) {
    // Return the found joke as JSON
    res.json(findjoke);
  } else {
    // Return 404 error if joke not found
    res.status(404).json({ error: "Joke not found, please try again." })
  }
})
